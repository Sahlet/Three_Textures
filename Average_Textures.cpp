...
public:
	...
	inline void use(const T& texture, const node& n, const matrix< info >& infos) throw(exception){
		if (!can_be_used_again_) throw(exception("после отладки программы этот throw надо удалить"));
		if (!used){//первый раз выставляем уровень - значит надо определить taken_before
			...
			for (const auto& i : neighbors){
				...
				if (l & taken_before){
					...
					if (textures[l/2] != infos(i.x, i.y).textures[l/2]) throw(exception("после отладки программы этот throw надо удалить"));
				} else {
					taken_before |= l;
					textures[l/2] = infos(i.x, i.y).textures[l/2];
				}
			}
		}
